<h2>ShopperPress DataFeed Importer CSV File Uploader</h2>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
	...
	if($_POST['action'] == 'upload') 
	{
		if ($file['error'] == 0) 
		{
			...
			spdfi_createcsvprofile( $filename );// will create a new csv file profile with the new filename
			$csvprofile = spdfi_getcsvprofile( $filename );
			$profilename = spdfi_csvfilesprofilename($filename);
			...
			{				
				...
				{
					$csvprofile['format']['delimiter'] = $_POST['manualdelimiter'];
				}
				else
				{
					$csvprofile['format']['delimiter'] = ',';// default set as none submitted
				}

				echo '<div id="message" class="updated fade"><p>Success - CSV File Uploaded and renamed too ' . $filename . '</p><p>Please go to CSV Profile page and complete fill out all details required to create a permanent profile for your csv file.</div>';
			}
			...
			else
			{
				...
				{
					$csvprofile['format']['delimiter'] = $conf['sep'];
				}
				
				$csvprofile['format']['columns_pear'] = $conf['fields'];// add number of csv columns to profile
				$csvprofile['format']['quote_pear'] = $conf['quote'];// add  auto found quote to profile
	...
	update_option( $profilename, $csvprofile );				
} 
?>
			...
			<div id="poststuff">
				<div id="datafeed-upload" class="postbox">
				  <h3 class='hndle'><span>Delete Unused CSV Files</span></h3>
                        ...
                        $csvfiles_dir2 = spdfi_getcsvfilesdir();
