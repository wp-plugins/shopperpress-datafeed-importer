...
function spdfi_optionsinstallation($state)
{
	...
	$exclusions = "'does','work','description','save','$ave','month!','year!','hundreds','dollars','per','month','year','and','or','but','at','in','on','to','from','is','a','an','am','for','of','the','are','home','much','more','&','every','this','has','been','with','selecting','set','other','thingy','maybe','thats','not','too','them','must-have','youre','can','these','where','will','our','end','all','using','use','your','get','getting','away','you','who','help','helps','any','plus','new','offer','fees','thinking','consider','going','into','where','interested','you'll','that's','fee's','year's','were','had','through','have','made','that','how','his','her','its','&amp;','&','&ndash;','&pos;'";
	...
	if($state == 1)// delete options before attempted re-adding them
	{
		...
		delete_option('spdfi_postsperhit_global');// usually 5-20
		...
		delete_option('spdfi_processingdelay');// length of time forced between processing events
		delete_option('spdfi_stage4fieldscolumns');// number of rows on stage 4 for pairing columns to custom fields
		delete_option('spdfi_lastpointofexecution');// indicates the last known point application reached during processing attempts
		delete_option('spdfi_randomdate_yearstart');// year for random date start
		delete_option('spdfi_randomdate_monthstart');// month for random date start
		delete_option('spdfi_randomdate_daystart');// day for random date start
		delete_option('spdfi_randomdate_yearend');// year for random date end
		delete_option('spdfi_randomdate_monthend');// month for random date end
		delete_option('spdfi_randomdate_dayend');// day for random date end
		delete_option('spdfi_itemqty');// item quantity setting NA will cause unlimited to be applied simply by not creating a custom field
	}
	...
	add_option('spdfi_postsperhit_global',3);// usually 5-20
	add_option('spdfi_publisherid',1);// any number
	add_option('spdfi_defaultcatparent','NA');// any number
	add_option('spdfi_maxstagtime',20);// usually 20-50
	...
	add_option('spdfi_processingdelay',5);// length of time forced between processing events
	add_option('spdfi_exclusions',$exclusions);// keyword and tag default exclusions
	add_option('spdfi_stage4fieldscolumns',10);// number of rows on stage 4 for pairing columns to custom fields
	add_option('spdfi_lastpointofexecution',0);// indicates the last known point application reached during processing attempts
	add_option('spdfi_randomdate_yearstart',2005);// year for random date start
	add_option('spdfi_randomdate_monthstart',09);// month for random date start
	add_option('spdfi_randomdate_daystart',12);// day for random date start
	add_option('spdfi_randomdate_yearend',2009);// year for random date end
	add_option('spdfi_randomdate_monthend',12);// month for random date end
	add_option('spdfi_randomdate_dayend',10);// day for random date end
	add_option('spdfi_characterencoding','utf8');// indicates what encoding type to use for imported post title and content
	add_option('spdfi_demomode',0);// 1 will put demo mode on and 0 is off
	add_option('spdfi_itemqty','NA');// item quantity setting NA will cause unlimited to be applied simply by not creating a custom field
